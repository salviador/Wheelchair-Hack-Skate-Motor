...
*/
    ...
    /*
    esp_ble_auth_req_t auth_req = ESP_LE_AUTH_NO_BOND; //  ESP_LE_AUTH_REQ_SC_MITM_BOND;     //bonding with peer device after authentication
    ...
    */
    ...
    BLE_SendNotification_Queue = xQueueCreate( 1, sizeof( uint32_t ) );
}
...
void Send_BLE_Notification_Task(void *pvParameter) {
    ...
    while(1){
        ...
        if (xQueueReceive(BLE_SendNotification_Queue, &data, portMAX_DELAY) == pdPASS ) {
            EventBits_t IS_BLE_CONNECT = xEventGroupGetBits(BLE_event_group);
            if(IS_BLE_CONNECT & BLECONNECTED_BIT){
                esp_ble_gatts_send_indicate(Joystick_profile_tab[0].gatts_if, Joystick_profile_tab[0].conn_id, BLE_wheels_handle_table[IDX_CHAR_VAL_A], 2, data, false);
            }
        }
